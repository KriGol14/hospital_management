...
String doc_name = temp.getAttribute("doc_name").toString();
...
if (now_date.compareTo(appointment_datetime_date) > 0){
	...
	if (Integer.parseInt(now_month) == Integer.parseInt(appointment_datetime_month)){
		// the apppointment can be canceled if it's three or more days later
		if (Integer.parseInt(appointment_datetime_day) - Integer.parseInt(now_day) >= 3){
			try {
				...
				PreparedStatement preparedStatement = connection.prepareStatement("delete from javabasics.arranged_appointments where doctor_name = ? and appointment_datetime = ?");
